...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[GHOST0] = variableDomainsDict[GHOST1] = [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[OBS0] = variableDomainsDict[OBS1] = range(X_RANGE + Y_RANGE + MAX_NOISE - 1)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for e in eliminationOrder:
            curr, temp = joinFactorsByVariable(curr, e)
            if len(temp.unconditionedVariables()) != 1:
                elim = eliminate(temp, e)
                curr.append(elim)
        res = normalize(joinFactors(curr))
        return res
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            if noisyDistance == None:
                return 0
        realDist = manhattanDistance(pacmanPosition, ghostPosition)
        res = busters.getObservationProbability(noisyDistance, realDist)
        return res
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in ghostPos:
            bef = bel[pos]
            llhd = self.getObservationProb(observation, pacmanPos, pos, jailPos)
            bel[pos] = llhd * bef
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        dist = DiscreteDistribution()
        for old in ghostPos:
            new = self.getPositionDistribution(gameState, old)
            for newPos, llhd in new.items():
                dist[newPos] += llhd * bel[old]
        self.beliefs = dist
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        dist = DiscreteDistribution()
        for p in self.particles:
            llhd = self.getObservationProb(observation, pacmanPos, p, jailPos)
            dist[p] += llhd
        dist.normalize()
        if dist.total() == 0:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for old in ghostPos:
            new = self.getPositionDistribution(gameState, old)
            for newPos, likelihood in new.items():
                dist[newPos] += likelihood * bel[old] 
        self.particles = []
        for _ in range(self.numParticles):
            self.particles.append(dist.sample())
        "*** END YOUR CODE HERE ***"
