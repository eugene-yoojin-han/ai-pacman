...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghost in livingGhostPositionDistributions:
            queue.push(ghost.argMax(), self.distancer.getDistance(pacmanPosition, ghost.argMax()))
        closest = queue.pop()
        curr = self.distancer.getDistance(pacmanPosition, closest)
        for move in [Directions.WEST, Directions.EAST, Directions.NORTH, Directions.SOUTH]:
            try:
                next = self.distancer.getDistance(closest, Actions.getSuccessor(pacmanPosition, move))
                if next < curr:
                    return move
            except Exception:
                pass
